...
namespace mgscript {
	...
	vector<string> lowerInterpret(string str) {
		if (!str.size()) {
			vector<string> empty;
			return empty;
		}
		...
		if (commandVec[0] == "FOR") {
			...
			for (auto i : iterable) {
				string calculationLive = currentLine;
				calculationLive = str_kit::replaceToken(calculationLive, lex, i);
				auto temp = lowerInterpret(calculationLive);
